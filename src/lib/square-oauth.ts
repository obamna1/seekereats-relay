...
const OAUTH_SCOPES = ['MERCHANT_PROFILE_READ', 'ITEMS_READ', 'ORDERS_WRITE', 'PAYMENTS_WRITE'].join(
  '+'
);
...
export async function exchangeCodeForToken(
  ...
  if (!response.ok || data.errors) {
    const errorMsg = data.errors?.[0]?.detail || 'Failed to exchange code for token';
...
export async function getMerchantInfo(
  ...
  if (!merchantResponse.ok || merchantData.errors) {
    throw new Error(merchantData.errors?.[0]?.detail || 'Failed to get merchant info');
  }
