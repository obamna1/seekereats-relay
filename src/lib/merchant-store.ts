 ...
 */
export async function storeMerchantTokens(tokens: MerchantTokens): Promise<void> {
  const store = await readStore();
  const key = tokens.isSandbox ? `sandbox:${tokens.merchantId}` : `prod:${tokens.merchantId}`;
 ...
 */
export async function getCurrentMerchantTokens(isSandbox: boolean): Promise<MerchantTokens | null> {
  const store = await readStore();
 ...
 * List all merchants
 */
export async function listMerchants(isSandbox?: boolean): Promise<MerchantTokens[]> {
  const store = await readStore();
 ...
 */
export async function deleteMerchantTokens(merchantId: string, isSandbox: boolean): Promise<void> {
  const store = await readStore();
  const key = isSandbox ? `sandbox:${merchantId}` : `prod:${merchantId}`;
  delete store.merchants[key];

  // Also clear current if it matches
  const currentKey = isSandbox ? 'current:sandbox' : 'current:prod';
  if (store.merchants[currentKey]?.merchantId === merchantId) {
    delete store.merchants[currentKey];
  }
